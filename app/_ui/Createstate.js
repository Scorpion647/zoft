...
export const CreatelargeAdmin = ({ sharedState, updateSharedState, onVisibilityChange }) => {
  ...
  const toggleCondition = () => {
    updateSharedState('conditionMet', (prev) => !prev);
  };
...
console.log(formattedValue); // "1234.56"
  ...
  const handleSubmit = async () => {
    ...
    for (const [index, row] of tableData.entries()) {
      ...
      const [record_position, material_code, bill_number, billed_quantity, subheading] = row;
      ...
      if (record_position && material_code && bill_number && billed_quantity && subheading) {
        ...
        const matchedRecord = await getRecord(orderNumber,pos);
            ...
            let Trm = sharedState.valorTRM;
            if( Trm === undefined || Trm === NaN || Trm === ""){
               Trm = 0;
            }
            const gross = ((((((hotInstance.getDataAtCell(index, 3)) / sharedState.columnSum) * 100)) * sharedState.pesototal) / 100).toFixed(2);
            const packag = (((((hotInstance.getDataAtCell(index, 3) / sharedState.columnSum) * 100)) * (sharedState.bultos)) / 100).toFixed(3);
            const conver = (((((((hotInstance.getDataAtCell(index, 3) / sharedState.columnSum) * 100)) * sharedState.pesototal) / 100)) / (hotInstance.getDataAtCell(index, 3))).toFixed(8)
    ...
    let totalSum = 0;
        ...
        for (let row = 0; row < data.length; row++) {
          ...
          if (unip !== "" && can !== "" && unip !== 0 && can !== 0) {
            if (sharedState.TRM) {
              tot = (unip * sharedState.valorTRM * can).toFixed(2);
            } else {
  ...
  const handleDataChange = (changes) => {
    if (changes) {
      ...
      changes.forEach(([row, col, oldValue, newValue]) => {
        ...
        if (col === 0) {
          updateSharedState('SelectedCellValue', newValue)
          calculateColumnSum();
        }
      });
      setData(newData);
  ...
  const handleSwitchChange = (e) => {
    updateSharedState('TRM', (e.target.checked));
};
  ...
  useEffect(() => {
    async function fetchData() {
      const records = await getRecords(1, 100); // Ajusta según necesites
      if (Array.isArray(records)) {
        const recordsInfo = await getRecordsInfo(1, 100); // Ajusta según necesites
  ...
  const filteredDataa = data.filter(item =>
    ...
    (selectedMonth === '' || item.fecha.split('/')[1] === selectedMonth)
  );
  ...
  if (!hola && (selectedSupplier2 != "")) {
    return <ReturnTable supplier={selectedSupplier2} />;
  }
                ...
                <VStack>
                  {filteredData.map((item) => (
                    <VStack w="100%" key={item.orden}>
                      <Button
                        onClick={() => handleSupplierClickk(item)}
                        whiteSpace="nowrap"
                        paddingRight={2}
                        paddingLeft={2}
            ...
            />
                ...
                <HStack className=" bg-white rounded-2xl" padding="3"  position="relative" width="45%" spacing={0}>
                  <VStack spacing={0} align="start" justify="start" width="30%" >
                    ...
                    <Text h="20%" className=" font-semibold" fontSize="70%">Total por material:</Text>
                  </VStack>
                  <VStack spacing={0} align="end" justify="end" width="70%"  >
                    ...
                    <Text h="20%" fontSize="70%">{formatMoney(sharedState.facttotal)}</Text>
              ...
              <Box>
                <HotTable
                        ...
                        for (const change of changes) {
                            ...
                            if(col === 2 || col === 3){
                              if (!isNaN(data[row][2]) && !isNaN(data[row][3])) { // Verifica que ambos valores sean números
                                ...
                                hot.setDataAtRowProp(row, 4, result); // Establece el resultado en la Columna 3
                              } else {
                  ...
                  cells={(row, col, prop) => {
                    ...
                    if (col === 0) {
                      if (data[row][0] !== "" && data[row][1] === undefined) {
                        cellProperties.renderer = (hotInstance, td, row, col, prop, value, cellProperties) => {
                          // Llama al renderer de texto base de Handsontable
                          Handsontable.renderers.TextRenderer(hotInstance, td, row, col, prop, value, cellProperties);
                          // Aplica el estilo a la celda

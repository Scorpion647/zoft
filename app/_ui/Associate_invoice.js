...
export const Associate_invoice = ({
  ...
  useEffect(() => {
    setpruebass(sharedState.TRMCOP);
    console.log("si se cambia tal parece");
            ...
            if (hola === Number(row[0])) {
              ...
              if (total_quantity > 0) {
                ...
                if (sharedState.TRM) {
                  ...
                  !sharedState.TRM &&
                  parseFloat(sharedState.TRMCOP) !== 0
                ) {
  ...
  const UpdateData = async () => {
    ...
    if (
      ...
      (!sharedState.TRM && (!sharedState.TRMCOP || sharedState.TRMCOP <= 0))
    ) {
    ...
    for (const [index, row] of tableData.entries()) {
      ...
      if (
        ...
        const factunitprice = parseFloat(
  ...
  const handleCellDoubleClick = async (event, coords, TD) => {
    ...
    if (
      ...
      const unit_price = convertCommaToDot(
        data[coords.row]?.[3]?.toString().trim(),
      );
      ...
      const factorPrice =
        sharedState.TRM ? unit_price * sharedState.valorTRM : unit_price;
  ...
  return (
    <div
      className={`relative p-4 bg-gradient-to-tr from-gray-200 to-gray-300 border h-full border-gray-300 text-center rounded-3xl shadow-md flex flex-col`}>
      <HStack position="relative" width="100%" height="20%">
        ...
        <VStack width="45%" spacing={0}>
          <HStack
            ...
            <VStack spacing={0} align="start" justify="start" width="30%">
              <Text h="20%" className=" font-semibold" fontSize="70%">
                Descripcion:
              </Text>
              <Text h="20%" className=" font-semibold" fontSize="70%">
                Cantidad OC:
            ...
            {!sharedState.TRM && (
              <HStack
                ...
                <Input
                  ...
                  min="1"
                  step="0.0000000001"
                  value={isTable !== "Create" ? sharedState.TRMCOP : undefined}
                  onBlur={handleTRMCOP}
                  h="25px"
                  width="190px"
                  bg="white"></Input>
        ...
        <VStack
          ...
          <HStack align="center" justify="center" height="20%">
            <VStack width="50%" align="start" justify="start">
              <Text fontSize="80%" className=" font-semibold">
                Peso Total
              </Text>
            </VStack>
            <VStack width="50%" align="end" justify="end">
              <Input
                isDisabled={!isActive}
                fontSize="80%"
                width="100%"
          ...
          <HStack align="center" justify="center" height="20%">
            <VStack width="50%" align="start" justify="start">
              <Text fontSize="80%" type="numeric" className=" font-semibold">
                Bultos
              </Text>
            </VStack>
            <VStack width="50%" align="end" justify="end">
      ...
      <HStack height="7%" spacing={3}>
        <HStack padding="1" spacing={3} width="60%">
          <Text className=" font-bold  " fontSize="90%">
            Proveedor
          </Text>
          <Text fontSize="90%">{sharedState.proveedor}</Text>
        </HStack>
              ...
              if (data[row][5] !== "**********") {
                if (
                  ...
                  cellProperties.renderer = (
                    ...
                    td.setAttribute(
                      "title",
                      "Campo no cumple el parámetro de 10 digitos",
                    );
            ...
            if (source === "edit" || source === "CopyPaste.paste") {
              ...
              for (const [rowIndex, colIndex, oldValue, newValue] of changes) {
                ...
                if (colIndex === 2 || colIndex === 3) {
                  const result =
                    data[rowIndex][2] *
                    parseFloat(String(data[rowIndex][3]).replace(/[$,]/g, ""));
                  ...
                  (async () => {
                    try {
                      ...
                      if (Number(records.item) === Number(pos)) {
                        const { material_code, unit_price, total_quantity } =
                          records;
                        if (total_quantity > 0) {
                          const materialDetails =
                            await getMaterial(material_code);
                          const subheading = materialDetails?.subheading || "";
                      ...
                      console.error(
                        `Error processing records for row ${row}:`,
                        error,
                      );
                    }
                  })(),
